  ...
  context 'supported operating systems' do
    ['RedHat',].each do |osfamily|
      describe "perlbrew::perl class with default parameters on #{osfamily}" do
        ...
        it { should contain_exec("install_perl_#{default_perl}").that_requires('Class[perlbrew::install]').with({
          :command  => "/bin/bash --login -c \"source #{default_perlbrew_root}/etc/bashrc; "\
        ...
        it { should contain_exec("switch_to_perl_#{default_perl}").that_requires("Exec[install_perl_#{default_perl}]").with({
          :command => "/bin/bash --login -c \"source /etc/profile; #{default_perlbrew_root}/bin/perlbrew switch perl-#{default_perl}\""
        }) }
        ...
        it { should contain_concat__fragment('perlbrew_manpath').that_requires("Exec[install_perl_#{default_perl}]").with({
          :target  => '/etc/profile.d/perlbrew.sh',
          :content => "export PERLBREW_MANPATH=\"#{default_perlbrew_root}/perls/perl-#{default_perl}/man\"",
          :order   => '02'
        }) }
        it { should contain_concat__fragment('perlbrew_path').that_requires("Exec[install_perl_#{default_perl}]").with({
          :target  => '/etc/profile.d/perlbrew.sh',
          :content => "export PERLBREW_PATH=\"#{default_perlbrew_root}/bin:#{default_perlbrew_root}/perls/perl-#{default_perl}/bin\"",
          :order   => '03'
        }) }
        it { should contain_concat__fragment('perlbrew_perl').that_requires("Exec[install_perl_#{default_perl}]").with({
          :target  => '/etc/profile.d/perlbrew.sh',
